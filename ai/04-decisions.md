...
- **Status**: Accepted
- **Context**: FreeAgent is accounting/bookkeeping-oriented. Its bank account balance can diverge from the online-bank balance when opening balances, missing feed entries, duplicates, or explanations are unreconciled. Plaid was evaluated as an aggregator, but real-bank access is not the right fit for the current UK personal-use path.
- **Decision**: Personal banking should use the most reliable practical sources available now: Snoop CSV for broad multi-bank exports and Monzo Google Sheets for Monzo current account/pot-level truth. Plaid remains parked under Settings > Integrations as an experimental connector, not a primary route. Future aggregator evaluation should start with GoCardless Bank Account Data, then compare TrueLayer, Yapily, Tink, Moneyhub, Salt Edge, and Plaid only if UK access, consent refresh, and pricing fit the local-first use case. Keep all connectors behind an app-owned `ExternalBankConnector` abstraction that returns normalized accounts, balances, transactions, consent status, cursor, and sync-health records.
- **Consequences**: Dashboard Cash Position and transaction analysis can move toward online-bank truth without depending on bookkeeping balances or a blocked aggregator. FreeAgent remains read-only and useful for Business/accounting imports. Monzo pots receive special treatment because a bill can fail when its assigned pot lacks funds even if household cash elsewhere is positive. Any multi-user/commercial account aggregation must use an authorised provider or a deliberate FCA AISP path; local/personal experiments must still avoid screen scraping and never persist raw bank credentials.
